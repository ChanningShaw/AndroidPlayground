...
import com.wedream.demo.algo.algo.DPAlgorithm
...
class DPAlgorithmActivity : CategoryActivity() {
    ...
    private fun buildData(): List<Category> {
        return DPAlgorithm.getModels().map {
            Category(it.name)
        }
    }
    ...
    override fun onCategoryClick(data: Category, pos: Int) {
        ...
        intent.putExtra(AlgorithmDetailActivity.KEY_ALGO_TYPE, AlgorithmDetailActivity.TYPE_DP)
