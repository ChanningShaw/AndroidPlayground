...
class StringAlgorithmActivity : CategoryActivity() {
    ...
    private fun buildData(): List<Category> {
        return StringAlgorithm.getModels().map {
            Category(it.name)
        }
    }
    ...
    override fun onCategoryClick(data: Category, pos: Int) {
        ...
        intent.putExtra(AlgorithmDetailActivity.KEY_ALGO_TYPE, AlgorithmDetailActivity.TYPE_STRING)
